...
def BM_LIST(url,name):
        ...
        for movieLink, movieName, movieCast in movies:
                ...
                addDirAndLoadImg(movieName,movieLink,29,cachedImgUrlJsonObj)
...
def MoviesList(url, mode):
        ...
        for movieLink, movieTitle in movies:
                ...
                addDir(unescape(movieTitle),movieLink.replace('" target="_blank',''),mode,'')
...
def retrieve_YT_UserPlaylists(YouTubeUserId, name='', startIndex=1, maxCount=50, pageNbr=0, ifTitleContains='', ifTitleNotContains=''):
        ...
        for playlist in playlists:
                ...
                if re.search('Sominal', YouTubeUserId):
                        addDirAndLoadImg(playListTitle, playlistID,62,cachedImgUrlJsonObj)
                else:
...
def saveImgUrlJsonObj(cachedImgUrlJsonObj):
...
def loadImgUrlJsonObj(cachedImgUrlJsonObj):
...
cachedImgUrlJsonObj = loadImgUrlJsonObj(cachedImgUrlJsonObj)

def addDirAndLoadImg(name,url,mode,cachedImgUrlJsonObj):
        u=sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+str(mode)+"&name="+urllib.quote_plus(name)
        ok=True
        
        if cachedImgUrlJsonObj == None or len(cachedImgUrlJsonObj) == 0:
                xbmc.executebuiltin("XBMC.Notification(Loading images...,Movie images are loaded into cache first time.,30000)")
...
saveImgUrlJsonObj(cachedImgUrlJsonObj)
