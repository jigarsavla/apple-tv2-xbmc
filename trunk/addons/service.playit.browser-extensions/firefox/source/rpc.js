 ...
 * });
 */
const xhrlib = require("api-utils/xhr");
...
rpc.ServiceProxy.prototype.__callMethod = function(methodName, params, successHandler, exceptionHandler, completeHandler){
	...
	try {
		...
		//Calls made with XMLHttpRequest ------------------------------------------------------------
			...
			if(params && (!(typeof params == 'object') || (typeof params == 'Date'))) //JSON-RPC 1.1 allows params to be a hash not just an array
			...
			//Prepare the JSON-RPC request
				...
				if(params)
					//Added by ajju to make sure dict object is sent if only argument of type map is passed to method
					if(params.length === 1)
			...
			if(this.__isAsynchronous){
						...
						if(xhr.readyState == 4){
							...
							//JSON-RPC
								...
								if(xhr.status == '200' && xhr.responseText != ''){
									...
									err = Error('Unable to send request: Network error');
									err.code = 101
									throw err
								}
							}
				...
				var xhrTimeout = timer.setTimeout(
						function(){
							xhr.abort();
						},10000);
	...
	for(var key in obj){
		if(!useHasOwn || obj.hasOwnProperty(key)){
			...
			if(typeof obj[key] == 'string'){
				...
				if(typeof obj[key].__jsonclass__ == 'Array'){
					//console.info('good1');
					if(obj[key].__jsonclass__[0] == 'Date'){
						//console.info('good2');
						if(typeof obj[key].__jsonclass__[1] == 'Array' && obj[key].__jsonclass__[1][0])
...
rpc.toQueryString = function(params){
	if(!(typeof params == 'object' || typeof params == 'Array') || typeof params == 'Date')
		...
		if(useHasOwn && params.hasOwnProperty(key)){
			...
			if(typeof params[key] == Array){
				for(var i = 0; i < params[key].length; i++){
					...
					if(typeof params[key][i] == 'Date')
						str += encodeURIComponent(rpc.dateToISO8601(params[key][i]));
					else if(typeof params[key][i] == 'object')
